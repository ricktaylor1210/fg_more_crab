...
- 当前分层评估：静态实现约 82%~86%，实机集成约 45%~55%，发布成熟度约 35%~45%；按稳定 V1.0 综合衡量约 60%~70%。静态覆盖率不等于可发布程度。
- TASK-16~23 已作为可并行扩展线进入路线图，覆盖蟹卵养殖、潮汐入侵、元素盾牌、大型沉船、结构谜题、深海终局、3D 资源和持久化 / 性能基础；扩展开发不受 TASK-01~15 验收门禁禁止。
...
- `docs/07_dev_roadmap.md`：TASK-01~15 主线与 TASK-16~23 并行扩展路线图。
...
- `docs/project_progress_analysis.md`：当前分层进度、证据、风险和发布门禁。
