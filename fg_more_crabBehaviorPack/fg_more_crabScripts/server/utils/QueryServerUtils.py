...
def SetEntityQueryAndNotifyToClient(entity_id, query_name, query_value):
    ...
    MobQueryValueMap.setdefault(entity_id, dict())[query_name] = query_value
    GetServerMainSystem().BroadcastToAllClient("OnChangeEntityQueryToAllClientServerEvent",
                                      {"entity_id": entity_id, "query_name": query_name, "query_value": query_value})


def SetEntityQueryAndNotifyToClientAndAutoRecover(entity_id, query_name, query_value, recover_time=0.09, recover_value=0):
    """
    ...
    DelaySetEntityQueryTimerDict[entity_id][query_name] = GetCompGameLevel().AddTimer(recover_time, SetEntityQueryAndNotifyToClient, entity_id, query_name,
                                                                                 recover_value)
