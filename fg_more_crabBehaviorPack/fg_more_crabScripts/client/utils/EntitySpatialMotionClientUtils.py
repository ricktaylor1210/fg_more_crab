# -*- coding: utf-8 -*-
from . import AttributeClientUtils
...
def GetEntityForwardPos(entity_id, distant, is_foot_pos=False, is_center_pos=False):
    ...
    根据传入的entity_id和distant，计算出实体视角前方distant距离处的坐标
        该方法依赖于AttributeClientUtils.GetEntityDir和AttributeClientUtils.GetEntityPos（或AttributeClientUtils.GetEntityFootPos）来获取实体当前的方向和位置。
        参数is_foot_pos用于决定是否获取实体脚下的坐标。当is_foot_pos为True时，使用AttributeClientUtils.GetEntityFootPos；否则使用AttributeClientUtils.GetEntityPos。
    ...
    if is_foot_pos:
        ...
        current_pos = AttributeClientUtils.GetEntityCenterPos(entity_id)
    else:
...
def GetEntityForwardMotionDir(entity_id, distant):
    ...
    根据传入的entity_id和distant，计算出实体视角前方distant距离处的方向（Dir）
        该方法依赖于AttributeClientUtils.GetEntityDir来获取实体当前的方向。
    ...
    :rtype: tuple[int or float,int or float,int or float] or None
    """
    current_dir = AttributeClientUtils.GetEntityDir(entity_id)
...
def CheckEntityAroundEntityList(main_entity_id, around_square, exclude_entity_list=None, has_tag="", has_not_tag="", exclude_family_list=None,
    ...
    """
    检测指定实体ID周围的其他实体
        该方法利用GameClientApi.GetCompGameLocalPlayer和GameClientApi.GetEntitiesAround接口，检测给定实体（main_entity_id）周围一定范围（around_square）内的其他实体。
    ...
    main_entity_center_pos = AttributeClientUtils.GetEntityCenterPos(main_entity_id)
    ...
    around_entity_list = CompFactory.CreateGame(main_entity_id).GetEntitiesAround(main_entity_id, int(math.ceil(around_square)), filters)
    ...
    for entity_id in around_entity_list:
        ...
        entity_center_pos = AttributeClientUtils.GetEntityCenterPos(entity_id)
...
def CheckEntitySectorAroundEntityListApi(main_entity_id, around_radius=6, radius_angle=60, exclude_entity_list=None, is_front=False, has_tag="", has_not_tag="",
    ...
    main_entity_dir = ClientApi.GetDirFromRot(
        (0, CompFactory.CreateRot(main_entity_id).GetRot()[1])) if is_front else AttributeClientUtils.GetEntityDir(main_entity_id)
    ...
    dear_list = []
    for entity_id in around_entity_list:
        if CompFactory.CreateEffect(entity_id).HasEffect("invisibility"):
            continue
        entity_center_pos = AttributeClientUtils.GetEntityCenterPos(entity_id)
        delta = Vector3(entity_center_pos) - Vector3(main_entity_center_pos)
...
def CheckEntitySectorByPosDirApi(center_pos,face_dir,candidate_list, around_radius=6, radius_angle=60):
    """
    根据传入的中心生物ID获取扇形范围内的其他生物
        该方法根据传入的中心pos（center_pos）、扇形范围的长度（around_radius）、扇形范围的角度（radius_angle）等参数，
        检测并返回在扇形范围内的其他生物的ID列表。
        可以通过exclude_entity_list参数来排除某些特定的生物。
        is_front参数用于控制是否只考虑水平方向，不考虑低头或抬头的角度。

    :param center_pos: 需要检测的中心pos
    :type center_pos: tuple[float, float, float] or list[float, float, float]
    :param face_dir: 需要检测的dir
    :type face_dir: tuple[float, float, float] or list[float, float, float]
    :param candidate_list: 需要检测的entity list
    ...
    for entity_id in candidate_list:
        ...
        if distance > around_radius:
            continue

        # 水平角度计算
        horizontal_delta = Vector3(delta[0], delta[1], 0)  # 只考虑水平面上的分量
        horizontal_dir = Vector3(face_dir[0], face_dir[1], 0)
        ...
        vertical_dir = Vector3(0, 0, face_dir[2])
...
def CalculateRectVertices(entity_id, width, length, height=4):
    ...
    if entity_rot is None:
        return None
    dir_x, dir_y, dir_z = ClientApi.GetDirFromRot((0, entity_rot[1]))
...
def DelaySetEntityMotion(delay_time, entity_list, motion):
    ...
    if entity_list in _delay_set_motion_timer_dict:
        GetCompGameLevel().CancelTimer(_delay_set_motion_timer_dict[entity_list])
        _delay_set_motion_timer_dict.pop(_delay_set_motion_timer_dict)
    _delay_set_motion_timer_dict[entity_list] = GetCompGameLevel().AddTimer(delay_time, SetEntityMotion, entity_list, motion)
...
def RemoveEntityAllOldMotion(entity_id):
    ...
    if AttributeClientUtils.CheckEntityIsPlayer(entity_id):
        for MID in comp_action_motion.GetPlayerMotions():
            comp_action_motion.RemovePlayerMotion(MID)
    else:
        for MID in comp_action_motion.GetEntityMotions():
...
def AddEntityTrackMotion(entity_id, target_pos, dura_time, start_pos=None, relative_coord=False, is_loop=False, target_rot=None, start_rot=None,
    ...
    comp_action_motion=CompFactory.CreateActorMotion(entity_id)
    if AttributeClientUtils.CheckEntityIsPlayer(entity_id):
        motion_id = comp_action_motion.AddPlayerTrackMotion(target_pos, dura_time, start_pos, relative_coord, is_loop, target_rot, start_rot,
                                                                        use_velocity_dir)
        comp_action_motion.StartPlayerMotion(motion_id)
        return motion_id if motion_id and motion_id != -1 else None
    motion_id = comp_action_motion.AddEntityTrackMotion(target_pos, dura_time, start_pos, relative_coord, is_loop, target_rot, start_rot,
                                                                    use_velocity_dir)
